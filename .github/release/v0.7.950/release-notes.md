...
1. Better local server/model support, allowing you to have a fully local TomoriBot stack incl. voice, video, embedding, and image (or, half of that)
2. Personal providers wherein you can use your own preferred provider and key when interacting with a TomoriBot anywhere
3. New TomoriBot default persona (for real this time)
## New Tomori Features
### Provider Pooling
...
- Allow users to specify context length to use through new `custom-endpoint command`
- Two new capability types added: Speech (TTS) and Transcription (STT), allowing you to register your own custom endpoints for them.
	- Added ready-to-use scripts in `/scripts/` for deploying your own WhisperX for STT, and Qwen3TTS, Chatterbox-Turbo, or IrodoriTTS for TTS (see `/help speech` and `/help transcription` for instructions).
...
The new `/personal provider` command group (mirroring `/config provider`) allows you to route requests to your own set API keys and providers, bypassing server quotas, cooldowns (might change later on), as well as stingy server admins that only use DeepSeek v3.2 *cough*.
...
- Added support for random macros such as `{{random::apple::banana}}` (single or double brace OK). This will be resolved as "apple" or "banana" randomly upon building context. Note that this will often invalidate cache for context below it, so recommended to put this in low depth eg. context-note with low depth if you want maximum savings/PP.
...
- Consolidated `/samplers` command into one (due to Discord command limit issues).
  - /samplers now allow you to configure thinking level of the provider chosen.
...
- Bug wherein sending a follow-up after replying to a user impersonation, it would fall it back to the main persona talking instead of the user impersonation. 
