...
| `pin_selected_message` | `{pin_tool}` | `pin_message_enabled`；`MANAGE_MESSAGES` | 現在のチャンネルで重要な最近のメッセージをピン留めする。 |
...
| `refresh_message_timestamps` | `{timestamp_refresh_tool}` | ベースツール | 全メッセージに正確なタイムスタンプを付けて最近のコンテキストを再構築する。 |
...
| `process_youtube_video` | `{youtube_tool}` | ベースツール；YouTube/動画サポートのあるモデルが必要 | 特定のYouTubeリンクをオンデマンドで分析する。 |
| `analyze_image` | `{image_analysis_tool}` | ベースツール；設定済み`vision_llm`が必要；現在のチャットモデルが画像を認識できない場合のみ表示 | 画像理解を別のビジョンモデルに委譲する。 |
| `generate_image` | `{image_generation_tool}` | `imagegen_enabled`；アクティブなプロバイダーがネイティブ画像生成をサポートしている必要がある | 現在のプロバイダーで画像を生成または編集する。 |
| `generate_image_nai` | `{anime_image_generation_tool}` | `imagegen_enabled`；NovelAIプロバイダーまたはNovelAI任意APIキー | NovelAIでアニメスタイルの画像を生成または編集する。 |
| `generate_voice_message` | `{voice_message_tool}` | ElevenLabs任意APIキー；アクティブなペルソナにElevenLabsボイスが必要；`voice_message_enabled` | プレーンテキストの代わりにDiscordの音声返答を送信する。 |
