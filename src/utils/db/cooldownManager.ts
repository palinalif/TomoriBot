...
export async function checkMessageTriggerCooldownWithWhitelist(
	...
	const effectiveCooldownType = whitelistStatus.isChannelWhitelisted
	...
	if (whitelistStatus.isChannelWhitelisted) {
		log.info(`[Cooldown Check] Channel ${channelId} is whitelisted - using channel-specific cooldown type ${effectiveCooldownType}, length ${whitelistStatus.channelCooldownLength}s`);
	} else {
		log.info(`[Cooldown Check] Channel ${channelId} NOT whitelisted - using global cooldown type ${effectiveCooldownType}`);
	}
...
export async function setMessageTriggerCooldownWithWhitelist(
	...
	const effectiveCooldownLength = whitelistStatus.isChannelWhitelisted
		? (whitelistStatus.channelCooldownLength ?? globalCooldownLength)
		: globalCooldownLength;

	// Diagnostic logging for whitelist-based cooldowns
	if (whitelistStatus.isChannelWhitelisted) {
		log.info(`[Cooldown Set] Channel ${channelId} is whitelisted - setting cooldown type ${effectiveCooldownType}, length ${effectiveCooldownLength}s`);
	}
