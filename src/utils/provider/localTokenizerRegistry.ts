...
function detectTokenizerJsonMode(
	...
	if (
		...
		Object.keys(vocab).some(
			(token) =>
				token.includes("▁") || /<0x[0-9A-Fa-f]{2}>/.test(token),
		)
	) {
...
function collectSplitRegexSources(
...
function buildByteLevelCharToByteMap(): Map<string, number> {
	...
	return new Map(
		bytes.map((value, index) => [String.fromCodePoint(codePoints[index]), value]),
	);
}
