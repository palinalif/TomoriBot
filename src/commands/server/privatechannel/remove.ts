...
/**
 * Configures the `/server privatechannel remove` subcommand.
...
export const configureSubcommand = (subcommand: SlashCommandSubcommandBuilder) =>
  subcommand
    .setName("remove")
    .setDescription(localizer("en-US", "commands.server.privatechannel.remove.description"))
    .addChannelOption((option) =>
      option
        .setName("channel")
        .setDescription(localizer("en-US", "commands.server.privatechannel.remove.channel_description"))
        .addChannelTypes(ChannelType.GuildText)
        .setRequired(true),
    );

/**
 * Removes a channel from the private channel list, restoring normal STM sharing and thought log behavior.
...
export async function execute(
  ...
  try {
    ...
    // 3. Check if the channel is actually in the private channel list
    const currentChannels = tomoriState.config.private_channel_ids || [];
    if (!currentChannels.includes(channel.id)) {
      await replyInfoEmbed(interaction, locale, {
        titleKey: "commands.server.privatechannel.remove.not_found_title",
        descriptionKey: "commands.server.privatechannel.remove.not_found_description",
        descriptionVars: {
    ...
    await log.error("Error in /server privatechannel remove command", error, context);
    await replyInfoEmbed(interaction, locale, {
