...
const MODAL_CUSTOM_ID = "server_rpchannel_remove_modal";
const CHANNEL_CHECKBOX_ID_PREFIX = "server_rpchannel_checkbox_group";
...
export const configureSubcommand = (subcommand: SlashCommandSubcommandBuilder) =>
  subcommand.setName("remove").setDescription(localizer("en-US", "commands.server.rpchannel.remove.description"));
...
export async function execute(
  ...
  try {
    ...
    const currentChannels = tomoriState.config.rp_channel_ids ?? [];
    if (currentChannels.length === 0) {
      await replyInfoEmbed(interaction, locale, {
        titleKey: "commands.server.rpchannel.remove.no_channels_title",
        descriptionKey: "commands.server.rpchannel.remove.no_channels_description",
    ...
    if (groupCount > MAX_GROUPS_PER_MODAL) {
      await replyInfoEmbed(interaction, locale, {
        titleKey: "commands.server.rpchannel.remove.too_many_entries_title",
        descriptionKey: "commands.server.rpchannel.remove.too_many_entries_description",
        descriptionVars: {
    ...
    const modalResult = await promptWithRawModal(
      ...
      locale,
      {
        ...
        modalTitleKey: "commands.server.rpchannel.remove.modal_title",
    ...
    if (modalResult.outcome !== "submit") {
      log.info(`RP channel removal modal ${modalResult.outcome} for user ${userData.user_id}`);
      return;
    }

    if (!modalResult.interaction) {
      log.error("RP channel removal modal unexpectedly missing interaction");
      return;
    }
    ...
    if (channelsToRemove.length === 0) {
      await replyInfoEmbed(modalResult.interaction, locale, {
        titleKey: "commands.server.rpchannel.remove.no_changes_title",
        descriptionKey: "commands.server.rpchannel.remove.no_changes_description",
    ...
    const [updatedRow] = await sql`
      ...
      SET rp_channel_ids = ${formatTextArrayLiteral(updatedChannels)}::text[]
    ...
    if (!updatedRow) {
      const context: ErrorContext = {
        ...
        metadata: {
          command: "server rpchannel remove",
    ...
    if (!validatedConfig.success) {
      ...
      await log.error("Failed to validate updated config after rpchannel remove", validatedConfig.error, context);
      await replyInfoEmbed(modalResult.interaction, locale, {
        titleKey: "general.errors.update_failed_title",
        descriptionKey: "general.errors.update_failed_description",
        color: ColorCode.ERROR,
    ...
    const remainingCount = validatedConfig.data.rp_channel_ids.length;

    await replyInfoEmbed(modalResult.interaction, locale, {
      titleKey: "commands.server.rpchannel.remove.success_title",
      descriptionKey: "commands.server.rpchannel.remove.success_description",
      descriptionVars: {
    ...
    log.info(`RP channels removed in guild ${interaction.guildId}: ${channelsToRemove.join(", ")}`);
  } catch (error) {
    const context: ErrorContext = {
    ...
    await log.error("Error in /server rpchannel remove command", error, context);
...
async function buildCheckboxGroups(
  ...
  for (let i = 0; i < channelIds.length; i += MAX_OPTIONS_PER_GROUP) {
    ...
    checkboxGroups.push({
      ...
      labelKey:
        groupIndex === 0
          ? "commands.server.rpchannel.remove.checkbox_label"
          : "commands.server.rpchannel.remove.checkbox_label_continued",
      descriptionKey: groupIndex === 0 ? "commands.server.rpchannel.remove.checkbox_description" : undefined,
      minValues: 0,
      required: false,
      options,
    });
