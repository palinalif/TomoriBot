...
export const configureSubcommand = (
	...
	subcommand
		.setName("servermemoryadd")
		.setDescription(
			localizer("en-US", "commands.teach.servermemoryadd.command_description"),
		)
		.setDescriptionLocalizations({
			ja: localizer("ja", "commands.teach.servermemoryadd.command_description"),
		});
...
export async function execute(
	...
	try {
		...
		if (
			...
			await replyInfoEmbed(interaction, locale, {
				titleKey: "commands.teach.servermemoryadd.teaching_disabled_title",
				descriptionKey:
					"commands.teach.servermemoryadd.teaching_disabled_description",
				color: ColorCode.ERROR,
				flags: MessageFlags.Ephemeral,
			});
			return;
		...
		modalResult = await promptWithModal(interaction, locale, {
			...
			modalTitleKey: "commands.teach.servermemoryadd.modal_title",
			inputs: [
				{
					customId: MEMORY_INPUT_ID,
					labelKey: "commands.teach.servermemoryadd.memory_input_label",
		...
		if (existingMemory) {
			await replyInfoEmbed(modalSubmitInteraction, locale, {
				titleKey: "commands.teach.servermemoryadd.duplicate_title",
				descriptionKey: "commands.teach.servermemoryadd.duplicate_description",
		...
		await replyInfoEmbed(modalSubmitInteraction, locale, {
			titleKey: "commands.teach.servermemoryadd.success_title",
			descriptionKey: "commands.teach.servermemoryadd.success_description",
			descriptionVars: {
				memory:
		...
		await log.error("Error in /teach servermemoryadd command", error, context);
		...
		if (errorReplyInteraction) {
			try {
				...
				log.error(
					"Failed to send error reply in servermemoryadd catch block",
					replyError,
			...
			log.warn(
				"Interaction was not replied or deferred in servermemoryadd catch block, cannot send error message to user.",
				context,
			);
		}
	}
