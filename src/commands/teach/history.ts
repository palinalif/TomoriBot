			...
			embeds: [
				new EmbedBuilder()
					...
					.setDescription(
						localizer(
							locale,
							"commands.teach.history.no_messages_description",
						),
					)
			...
			embeds: [
				new EmbedBuilder()
					.setDescription(
						localizer(
							locale,
							"commands.teach.history.progress_extracting",
							{
	...
	try {
		...
		if (!tomoriState.llm.supports_structoutput) {
			await replyInfoEmbed(interaction, locale, {
				...
				descriptionKey:
					"commands.teach.history.model_incompatible_description",
				color: ColorCode.ERROR,
				flags: MessageFlags.Ephemeral,
			});
			return;
		...
		if (!embeddingModelId) {
			await replyInfoEmbed(interaction, locale, {
				...
				descriptionKey:
					"commands.teach.history.no_embedding_model_description",
				color: ColorCode.ERROR,
				flags: MessageFlags.Ephemeral,
			});
			return;
		...
		if (scope === "persona") {
					...
					embeds: [
						new EmbedBuilder()
							.setTitle(
								localizer(
									locale,
									"commands.teach.history.duplicate_title",
								),
							)
							.setDescription(
								localizer(
			...
			await personaSelectionInteraction.editReply({
				embeds: [
					new EmbedBuilder()
						.setTitle(
							localizer(locale, "commands.teach.history.success_title"),
						)
						.setDescription(
							localizer(
								locale,
								"commands.teach.history.success_description",
								{
					...
					.setDescription(
						localizer(
							...
							"commands.teach.history.success_automatic_description",
							{
								...
								message_count:
									formattedResult.messageCount.toString(),
