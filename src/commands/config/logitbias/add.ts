...
export const configureSubcommand = (
	...
	subcommand
		.setName("add")
		.setDescription(
			localizer("en-US", "commands.config.logitbias.add.description"),
		);
...
export async function execute(
	...
	try {
		...
		const oversizedTerm = terms.find(
			(term) => term.length > LOGIT_BIAS_TEXT_MAX_LENGTH,
		);
		if (oversizedTerm) {
			await replyInfoEmbed(modalInteraction, locale, {
				titleKey: "commands.config.logitbias.add.term_too_long_title",
				descriptionKey:
					"commands.config.logitbias.add.term_too_long_description",
				descriptionVars: {
		...
		if (biasValue === null) {
			await replyInfoEmbed(modalInteraction, locale, {
				...
				descriptionKey:
					"commands.config.logitbias.add.invalid_bias_description",
				descriptionVars: {
		...
		if (merged.addedCount === 0 && merged.updatedCount === 0) {
			await replyInfoEmbed(modalInteraction, locale, {
				...
				descriptionKey:
					"commands.config.logitbias.add.already_set_description",
		...
		await log.error(
			"Error executing /config logitbias add",
			error as Error,
			context,
		);
