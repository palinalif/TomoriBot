	...
	subcommand
		...
		.addNumberOption((option) =>
			option
				.setName("value")
				.setDescription(
					localizer("en-US", "commands.config.params.presence-penalty.value_description"),
				)
...
export async function execute(
	...
	try {
		...
		const [updatedRow] = await sql`
            ...
            SET llm_presence_penalty = ${newValue}
		...
		if (!validatedConfig.success || !updatedRow) {
			...
			await log.error(
				"Failed to update or validate llm_presence_penalty config",
		...
		await log.error(
			`Error executing /config params presence-penalty for user ${userData.user_disc_id}`,
			error as Error,
			context,
		);
