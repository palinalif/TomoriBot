...
export const configureSubcommand = (
  ...
  subcommand
    ...
    .setDescription(
      localizer(
        "en-US",
        "commands.config.params.frequency-penalty.description",
      ),
    )
    .addNumberOption((option) =>
      option
        ...
        .setDescription(
          localizer(
            "en-US",
            "commands.config.params.frequency-penalty.value_description",
          ),
        )
...
export async function execute(
  ...
  try {
    ...
    const currentValue =
      tomoriState.config.llm_frequency_penalty ?? FREQUENCY_PENALTY_DEFAULT;
    if (Math.abs(newValue - currentValue) < 0.001) {
      await replyInfoEmbed(interaction, locale, {
        titleKey: "commands.config.params.frequency-penalty.already_set_title",
        descriptionKey:
          "commands.config.params.frequency-penalty.already_set_description",
        descriptionVars: {
    ...
    await replyInfoEmbed(interaction, locale, {
      ...
      descriptionKey:
        "commands.config.params.frequency-penalty.success_description",
      descriptionVars: {
        frequency_penalty: newValue.toFixed(2),
