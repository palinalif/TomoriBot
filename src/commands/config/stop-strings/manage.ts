...
export const configureSubcommand = (subcommand: SlashCommandSubcommandBuilder) =>
  subcommand.setName("manage").setDescription(localizer("en-US", "commands.config.stop-strings.manage.description"));
  ...
  try {
    ...
    if (!speakerPatternChanged && !stopStringsChanged) {
      await replyInfoEmbed(modalInteraction, locale, {
        titleKey: "commands.config.stop-strings.manage.no_changes_title",
        descriptionKey: "commands.config.stop-strings.manage.no_changes_description",
        color: ColorCode.WARN,
      });
      return;
    }
...
function buildModalComponents(
  ...
  const components: ModalComponent[] = [
    {
      ...
      labelKey: "commands.config.stop-strings.manage.speaker_pattern_checkbox_label",
      descriptionKey: "commands.config.stop-strings.manage.speaker_pattern_checkbox_description",
  ...
  for (let index = 0; index < stopStrings.length; index += STOP_STRING_OPTIONS_PER_GROUP) {
    ...
    const options: CheckboxGroupOption[] = chunk.map((stop, chunkIndex) => {
      ...
      return {
        ...
        description: safeSelectOptionText(
          localizer(locale, "commands.config.stop-strings.manage.stop_string_option_description", {
    ...
    components.push({
      ...
      labelKey:
        groupIndex === 0
          ? "commands.config.stop-strings.manage.stop_strings_checkbox_label"
          : "commands.config.stop-strings.manage.stop_strings_checkbox_label_continued",
      descriptionKey:
        groupIndex === 0 ? "commands.config.stop-strings.manage.stop_strings_checkbox_description" : undefined,
      minValues: 0,
...
function formatStopStringList(stops: readonly string[], locale: string): string {
  ...
  if (stops.length > visibleStops.length) {
    visibleStops.push(
      localizer(locale, "commands.config.stop-strings.manage.more_removed", {
