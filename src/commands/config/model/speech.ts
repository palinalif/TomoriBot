...
export const configureSubcommand = (subcommand: SlashCommandSubcommandBuilder) =>
  subcommand.setName("speech").setDescription(localizer("en-US", "commands.config.model.speech.description"));
...
export async function execute(
  ...
  try {
    ...
    if (endpoints.length === 0) {
      await replyInfoEmbed(interaction, locale, {
        titleKey: "commands.config.model.speech.no_endpoints_title",
        descriptionKey: "commands.config.model.speech.no_endpoints_description",
    ...
    const options: SelectOption[] = endpoints.map((endpoint) => ({
      ...
      description: safeSelectOptionText(
        localizer(locale, "commands.config.model.speech.endpoint_description", {
          label: endpoint.label,
          api_style: endpoint.api_style,
          active: endpoint.is_default ? localizer(locale, "commands.config.model.speech.active_marker") : "",
        }),
      ),
    }));
    ...
    const modalResult = await promptWithPaginatedModal(interaction, locale, {
      ...
      modalTitleKey: "commands.config.model.speech.modal_title",
      components: [
        {
          customId: ENDPOINT_SELECT_ID,
          labelKey: "commands.config.model.speech.select_label",
          descriptionKey: "commands.config.model.speech.select_description",
          placeholder: "commands.config.model.speech.select_placeholder",
          required: true,
          options,
        },
      ],
    ...
    if (previousEndpoint?.custom_endpoint_id === selectedEndpoint.custom_endpoint_id) {
      await replyInfoEmbed(modalResult.interaction, locale, {
        titleKey: "commands.config.model.speech.already_selected_title",
        descriptionKey: "commands.config.model.speech.already_selected_description",
    ...
    await replyInfoEmbed(modalResult.interaction, locale, {
      titleKey: "commands.config.model.speech.success_title",
      descriptionKey:
        previousEndpoint && previousEndpoint.api_style !== selectedEndpoint.api_style
          ? "commands.config.model.speech.success_source_changed_description"
          : "commands.config.model.speech.success_description",
      descriptionVars: {
