...
export const configureSubcommand = (subcommand: SlashCommandSubcommandBuilder) =>
  subcommand
    .setName("transcripts")
    .setDescription(localizer("en-US", "commands.config.voice.transcripts.description"))
    .addStringOption((option) =>
      option
        .setName("set")
        .setDescription(localizer("en-US", "commands.config.voice.transcripts.set_description"))
        .setRequired(true)
        .addChoices(
          {
...
export async function execute(
  ...
  try {
    ...
    if (currentSetting === isEnabled) {
      await replyInfoEmbed(interaction, locale, {
        titleKey: "commands.config.voice.transcripts.already_set_title",
    ...
    await log.error("Error in /config voice transcripts command", error as Error, context);
