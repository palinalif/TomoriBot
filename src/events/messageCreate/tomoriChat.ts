...
import { BaseGuildTextChannel, DMChannel, MessageFlags } from "discord.js"; // Import value for instanceof check
import {
	getGeminiTools,
...
export default async function tomoriChat(
	...
	try {
		try {
			...
			if (tomoriState.config.humanizer_degree >= HumanizerLevel.LIGHT) {
				log.info(
