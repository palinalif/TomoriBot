...
export class CustomStreamAdapter implements StreamProvider {
  ...
  async *startStream(
    ...
    try {
      ...
      if (this.speakerGuardEnabled) {
        log.info(
          "CustomStreamAdapter: Speaker-boundary fallback guard enabled",
        );
      }
  ...
  private appendToStreamedTextTail(text: string): void {
    ...
    if (
      this.streamedTextTail.length > CustomStreamAdapter.STREAM_TEXT_TAIL_CHARS
    ) {
  ...
  private applySpeakerBoundaryFallbackGuard(chunk: OpenAIStreamChunk): {
    chunk: OpenAIStreamChunk;
  ...
  private async assembleOpenAIContext(
    ...
    for (const item of contextItems) {
      ...
      if (
        ...
        (item.role === "user" &&
          item.metadataTag &&
          CustomStreamAdapter.SYSTEM_INSTRUCTION_TAGS.includes(
            item.metadataTag,
          ))
      ) {
