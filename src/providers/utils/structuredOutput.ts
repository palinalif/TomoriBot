 ...
 * Supports both expression-classification (vision) and generic schema-based use cases.
 */
 ...
 *
 * @param request - Request parameters including API key, model, and prompts
 * @param responseSchema - JSON schema object for Google's responseSchema
 * @param zodSchema - Zod schema for validating the parsed JSON response
 * @returns Promise with validated result or error
 */
export async function callGoogleStructuredJSON<T>(
	...
	try {
		...
		// 3. Fetch and convert any images to base64 inlineData format
		if (request.images) {
		...
		// 4. Build generation config with structured output
		...
		// 5. Build content structure for Google API
		const contents: Content = { role: "user", parts };

		// 6. Call Google Gemini with structured output
		...
		if (!responseText) {
			...
			return { success: false, error: "Google returned an empty structured output response." };
		}
		...
		if (!validationResult.success) {
			...
			return { success: false, error: `Invalid response structure: ${validationResult.error.message}` };
		}
 ...
 *
 * @param request - Request parameters including API key, model, and prompts
 * @param responseSchema - JSON schema object for OpenRouter's json_schema.schema
 * @param zodSchema - Zod schema for validating the parsed JSON response
 * @param schemaName - Name for OpenRouter's json_schema.name field
 * @returns Promise with validated result or error
 */
export async function callOpenrouterStructuredJSON<T>(
	...
	try {
		...
		// 1. Build content parts array with user prompt text
		const contentParts: OpenrouterContentPart[] = [
			{ type: "text", text: request.userPrompt },
		];

		// 2. Fetch and convert images to base64 data URLs if provided
		if (request.images) {
			for (const image of request.images) {
				try {
					const response = await fetch(image.url);
		...
		if (!response.ok) {
			...
			return { success: false, error: `OpenRouter request failed: ${response.status} ${response.statusText}` };
		}
		...
		try {
			...
			return { success: false, error: "Invalid JSON response from OpenRouter." };
		}
