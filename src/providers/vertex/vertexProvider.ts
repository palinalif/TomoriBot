...
async function getDefaultVertexModel(): Promise<string> {
	...
	try {
		const availableModels =
			await loadAvailableModelsForProvider(providerName);
	...
	async validateApiKey(
		...
		try {
			...
			if (!responseText?.toLowerCase().includes("valid")) {
				log.warn(
					"Vertex validation response did not contain 'VALID'",
				);
				const adapter = new VertexStreamAdapter();
	...
	async getTools(
		...
		try {
			const toolStateForContext: ToolStateForContext = {
				...
				llm: {
					...
					supports_structoutput:
						tomoriState.llm.supports_structoutput,
				},
				config: {
					...
					self_teaching_enabled:
						tomoriState.config.self_teaching_enabled,
					...
					nai_exclusive_imggen:
						tomoriState.config.nai_exclusive_imggen,
					voice_message_enabled:
						tomoriState.config.voice_message_enabled,
				},
			};
			...
			const allToolsConfig =
				await vertexAdapter.getAllToolsInProviderFormat(
					finalBuiltInTools,
	...
	async streamToDiscord(
		...
		try {
			...
			const streamConfig: VertexStreamConfig = {
				...vertexConfig,
				maxMessageLength:
					DISCORD_STREAMING_CONSTANTS.MAX_SINGLE_MESSAGE_LENGTH,
				flushBufferSize:
				...
				maxTypingTimeMs:
					DISCORD_STREAMING_CONSTANTS.MAX_TYPING_TIME_MS,
			...
			const streamContext: StreamContext = {
				...
				rotationKeyRetriesUsed:
					streamingContext?.rotationKeyRetriesUsed,
