...
export default {
  general: {
    ...
    errors: {
      ...
      voice_transcription_unavailable_description: `ここではまだ音声文字起こしを利用できません。\`/optional-key elevenlabs set\` でElevenLabsキーを設定してから、もう一度お試しください。`,
  ...
  genai: {
    ...
    message_interaction: {
      reply_context_title: `返信コンテキスト`,
      reply_context_description: `このメッセージは[以前のメッセージ]({message_url})への返信です。`,
      reply_context_footer: `以前のメッセージを書いた {user} に返信中`,
    },
    ...
    stream: {
      ...
      prohibited_content_admin_notice_description:
        "確認: メッセージ(`/tool refresh`)、記憶/設定(`/memory personal export`、`/memory server export`、`/server config export`)、問題のあるメンバーをブラックリスト(`/server user-blacklist add`)、またはプロバイダを変更(`/config model`)",
      ...
      rate_limit_tip:
        "数分お待ちいただいてから再度お試しください。複数の個人キーをお持ちなら、`/config api-key rotation` の利用も検討してください。",
      model_fallback_hint: "耐障害性を高めるには、`/config model fallback` でモデルのフェイルオーバーを設定できます。",

      content_blocked_title: "🔴️ プロバイダーコンテンツフィルター",
      content_blocked_tip:
        "ヒント: `/config jailbreaks` でこのエラーの回避を試すか、メッセージ(`/tool refresh`)、記憶/設定(`/memory personal export`、`/memory server export`、`/server config export`)、問題のあるメンバーをブラックリスト(`/server user-blacklist add`)、またはプロバイダを変更(`/config model`)を確認してください。",
...
- 自動トリガーチャンネルを制限する`,
      ...
      status: {
        ...
        not_available: `N/A`,
        see_all_memories_prompt: `すべての記憶を表示するには \`/memory personal export\` または \`/memory server export\` を使用してください`,
        memories_omitted: `...他 {count} 件の記憶が省略されました`,
        export_footer: `完全な記憶を表示するには \`/memory personal export\` または \`/memory server export\` を使用してください`,
        export_footer_full: `すべての詳細を表示するには \`/memory personal export\`、\`/memory server export\`、または \`/server config export\` を使用してください`,
...
- \`/conditioning reward\`コマンド（headpat、hug、kiss、tickle）でインタラクティブなご褒美モーメント`,
        imagegen_title: `画像生成`,
...
**APIキー / Bearerトークン**
オプションです。セットアップ後に \`/config api-key set\` で認証トークンを保存できます。
...
- ほとんどの用途でSonnetが最もコストパフォーマンスに優れています`,
      ...
      elevenlabs: {
        ...
        what_is_description: `ElevenLabsはオプションの音声合成（TTS）プロバイダーです。`,
...
- ペルソナをエクスポートしてバックアップするか、他のサーバーと共有！`,
    ...
    bot: {
      ...
      respond: {
        ...
        main_persona_description: `メインペルソナ`,
        alter_persona_description: `オルタペルソナ`,
        embed_hide_notice: `\`/config bot-permissions\` でこの埋め込みを非表示にできます。`,
      ...
      impersonate: {
        ...
        persona_impersonation_notice_description: `\`/config bot-permissions\`で「なりすまし埋め込みを非表示」権限を有効にすると、この埋め込みを非表示にできます。`,
        persona_impersonation_notice_footer: `{user}によるなりすまし`,
        user_impersonation_notice_description: `\`/config bot-permissions\`で「なりすまし埋め込みを非表示」権限を有効にすると、この埋め込みを非表示にできます。`,
    ...
    conditioning: {
      ...
      toggle: {
        description: `このサーバー内の全ペルソナに対する条件付けプロンプト注入を切り替えます。`,
        type_description: `管理する条件付けの種類。`,
        type_choice_reward: `ご褒美`,
        type_choice_punish: `おしおき`,
        enabled_description: `この条件付け種類をプロンプトに注入するかどうか。`,
        already_title: `変更はありません`,
        already_enabled_description: `サーバー全体の{type_label}は、すべてのペルソナですでに有効です。`,
        already_disabled_description: `サーバー全体の{type_label}は、すべてのペルソナですでに無効です。新しい記録は引き続き保存されます。`,
    ...
    config: {
      ...
      "tool-notices": {
        description: `チャットに表示するツール通知埋め込みを管理します。`,
        visibility: {
          description: `チャットに残すツール通知埋め込みを選択します。`,
          modal_title: `ツール通知の表示設定`,
          checkbox_label: `表示するツール通知`,
          checkbox_label_continued: `表示するツール通知（続き）`,
          checkbox_description: `チェックした通知はチャットに表示されます。外した通知は非表示になり、許可されていれば thoughtlogs に送られます。`,
          no_changes_title: `変更はありません`,
          no_changes_description: `ツール通知の表示設定はすでにその状態です。`,
          success_title: `ツール通知の表示設定を更新しました`,
          success_description: `今回非表示にした項目（{hidden_count}）: {hidden_list}\n今回再表示した項目（{restored_count}）: {restored_list}`,
          too_many_title: `通知タイプが多すぎます`,
          too_many_description: `設定されたツール通知タイプは {count} 件あり、モーダル上限の {max_entries} 件（{max_groups} グループ）を超えています。`,
      ...
      permissions: {
        ...
        permission_choice_pinmessage: "メッセージ管理",
        permission_choice_imagegen: "画像生成",
        permission_choice_videogen: "動画生成",
        ...
        pinmessage_desc: "最近のメッセージのピン留めと、Tomori自身のメッセージの編集・削除を許可",
      ...
      prompt: {
        ...
        change: {
          ...
          part1_description: `主な指示。Discord のモーダルでは各テキスト入力に文字数制限があるため、4つに分割しています。{bot} は私の名前、{user} はトリガーしたユーザーとして使用できます`,
          part1_placeholder: `例：{bot} はフレンドリーで親切...`,
          part2_label: `システムプロンプト（2/4）- 任意`,
          part2_description: `指示の続き。Discord のモーダル入力の文字数制限に対応するための分割です（任意）`,
          part2_placeholder: `追加の指示...`,
          part3_label: `システムプロンプト（3/4）- 任意`,
          part3_description: `指示の続き。Discord のモーダル入力の文字数制限に対応するための分割です（任意）`,
          part3_placeholder: `さらに指示...`,
          part4_label: `システムプロンプト（4/4）- 任意`,
          part4_description: `指示の続き。Discord のモーダル入力の文字数制限に対応するための分割です（任意）`,
    ...
    server: {
      ...
      quota: {
        ...
        videogen: {
          ...
          unlimited: `無制限`,
          daily_user_quota_description: `ユーザーごとの日次動画生成制限を設定します。`,
          ...
          serverwide_quota_description: `サーバー全体の動画生成制限の合計を設定します。`,
          ...
          serverwide_quota_resets_in_description: `サーバー全体の動画クォータがリセットされるまでの日数を設定します。`,
          serverwide_quota_resets_in_days_description: `リセットまでの日数（1-365、デフォルト: 365）。`,
          serverwide_quota_resets_in_success_title: `クォータリセット期間が更新されました`,
...
**警告:** 現在、このサーバーではパーソナライズが無効になっているため、この変更はここでの私の行動に影響しません。パーソナライズが有効になっている他のサーバーでは反映されます。`,
    ...
    generate: {
      image: {
        ...
        disabled_description:
          "このサーバーでは画像生成が無効になっています。`/config bot-permissions` で有効にできます（管理権限が必要）。",
        ...
        api_key_decrypt_failed_description:
          "APIキーの復号化に失敗しました。`/config api-key set`を使用して再設定してください。",
      ...
      video: {
        ...
        disabled_description:
          "このサーバーでは動画生成が無効になっています。`Manage Server`権限を持つメンバーが `/config bot-permissions` で有効にできます。",
        wrong_provider_title: "🔴 サポートされていないプロバイダー",
        wrong_provider_description:
          "動画生成にはGoogle、OpenRouter、またはZ.aiが必要です。現在のプロバイダーは **{current_provider}** です。",
        no_api_key_title: "🔴 APIキーがありません",
        no_api_key_description: "APIキーが設定されていません。`/config api-key set` を使用してください。",
        api_key_decrypt_failed_title: "🔴 APIキーエラー",
        ...
        no_video_model_description:
          "プロバイダーに動画モデルが設定されていません。`/config model video` で設定してください。",
        error_title: "🔴 動画生成に失敗しました",
        unsupported_provider_description: "プロバイダー **{provider}** では動画生成がサポートされていません。",
        no_data_description:
          "APIから動画データを受信できませんでした。生成がブロックされたか失敗した可能性があります。",
        file_too_large_title: "🔴 動画が大きすぎます",
        file_too_large_description:
          "生成された動画（{size_mb} MB）がDiscordの25 MBファイルサイズ制限を超えています。短いプロンプトや異なるアスペクト比をお試しください。",
        invalid_image_title: "🔴 無効な画像",
        invalid_image_description: "アップロードされた参照画像を処理できませんでした。別の画像をお試しください。",
        timeout_description:
          "動画生成がタイムアウトしました。プロバイダーに負荷がかかっている可能性があります。後でもう一度お試しください。",
        blocked_description:
          "プロバイダーのコンテンツ安全フィルターにより動画生成がブロックされました。別のプロンプトをお試しください。",
  ...
  events: {
    ...
    addBot: {
      ...
      rejoin_description: `このサーバーに再追加されたようです。以前の設定と人格はそのままです！\`/config\`、\`/persona\`、\`/memory\`、\`/server\`コマンドで私を管理できます。
