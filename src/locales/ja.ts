...
export default {
	general: {
		...
		// 一般的なエラーメッセージ
		errors: {
			command_timeout: `コマンドがタイムアウトしました。再度お試しください。`,
			guild_only_title: `サーバー専用コマンド`,
			guild_only_description: `このコマンドはサーバーチャンネル内でのみ使用できます。現在のところDMやグループチャットでは動作しません！`,
			dm_not_supported_title: `DMはサポートされていません`,
			dm_not_supported_description: `申し訳ありませんが、ダイレクトメッセージではなく、サーバーチャンネル内でのみチャットできます。将来的には変更される可能性がありますが、現在のところサーバーでのみ動作します！`,
...
Discord APIレイテンシ：\`{discord_response}ms\``,
		...
		config: {
			...
			humanizerdegree: {
				...
				modal_title: `ヒューマナイザー度設定`,
				select_label: `ヒューマナイザーレベル`,
				select_description: `Tomoriの応答がどの程度人間らしく感じるかを選択してください`,
				...
				desc_none: `ヒューマナイゼーションなし。標準的なAI応答で正式な口調と構造。`,
				desc_light: `人間らしい応答ガイドラインを追加。絵文字を制限（0-2個）、簡潔な応答を優先。`,
				desc_medium: `軽微機能＋タイピングシミュレーションと改善されたメッセージチャンクで自然な流れ。`,
				desc_heavy: `すべての機能＋カジュアルなテキスト処理（小文字、句読点の削減）で非公式な口調。`,
			...
			model: {
				...
				modal_title: `AIモデル選択`,
				select_label: `AIモデル`,
				select_description: `Tomoriが使用するAIモデルを選択してください`,
				...
				api_key_incompatible_title: `APIキーが互換性がありません`,
				...
				validation_error_description: `APIキー互換性の検証中にエラーが発生しました。再度お試しください。`,
...
{items}`,
			...
			apikeyset: {
				...
				provider_description: `APIキーのAIプロバイダーを選択してください`,
				provider_placeholder: `プロバイダーを選択...`,
				api_key_label: `APIキー`,
				api_key_description: `選択したプロバイダーのAPIキーを入力してください`,
				api_key_placeholder: `APIキーをここに貼り付けてください...`,
				no_providers_title: `利用可能なプロバイダーがありません`,
				no_providers_description: `データベースにAIプロバイダーがありません。ボット管理者にお問い合わせください。`,
				...
				unsupported_provider_description: `プロバイダー「{provider}」は現在APIキー検証でサポートされていません。`,
				validation_error_title: `検証エラー`,
				validation_error_description: `APIキーの検証中にエラーが発生しました。再度お試しください。`,
				key_validation_failed_title: `APIキー検証に失敗しました`,
				key_validation_failed_description: `提供されたAPIキーは{provider}に対して有効ではありません。キーを確認して再度お試しください。`,
			...
			setup: {
				...
				api_key_description: `有効なAPIキーを入力してください`,
				preset_label: `パーソナリティプリセット名`,
				preset_description: `適用するパーソナリティプリセットを選択してください`,
				preset_placeholder: `プリセットを選択...`,
			...
			preset: {
				...
				modal_title: `パーソナリティプリセット適用`,
				select_label: `パーソナリティプリセット`,
				select_description: `Tomoriに適用するプリセットを選択してください。現在の属性と対話が上書きされます。`,
				...
				success_description: `'{preset_name}' プリセットをTomoriに正常に適用しました。`,
			},
			tomoripermissions: {
		...
		teach: {
			sampledialogue: {
				...
				user_input_description: `ボットへのサンプル質問（例：好きな食べ物は何ですか？）`,
				user_input_placeholder: `好きな食べ物は何ですか？`,
				bot_input_label: `Tomoriの応答`,
				bot_input_description: `ボットがどのように応答すべきか（例：マ、マンゴーが好きです...）`,
				bot_input_placeholder: `マ、マンゴーが好きです...`,
...
> {bot_input}`,
			...
			attribute: {
				...
				modal_description: `Tomoriがこのサーバーで持つパーソナリティ特性（例：マンゴーが好き）`,
			...
			servermemory: {
				...
				modal_description: `TomoriBotがこのサーバーのみで覚えている記憶（例：このサーバーのメンバーはマンゴーが好き）`,
			...
			personalmemory: {
				...
				modal_title: `個人記憶を追加`,
				modal_description: `サーバーに関係なくTomoriBotがあなたについて覚える記憶（例：マンゴーが好き）`,
...
**警告：** このサーバーでは現在パーソナライゼーションが無効になっているため、この記憶はここでは使用されません。パーソナライゼーションが有効な他のサーバーでは引き続き利用できます。`,
				success_but_blacklisted_description: `'{memory}' をあなたの個人記憶に正常に追加しました。

**警告：** あなたは現在このサーバーでパーソナライゼーション機能からブラックリストに登録されているため、この記憶はここでは使用されません。ブラックリストに登録されていない他のサーバーでは引き続き利用できます。`,
			},
			nickname: {
...
**警告：** このサーバーでは現在パーソナライゼーションが無効になっているため、ここではこのニックネームを使用しません。パーソナライゼーションが有効な他のサーバーでは引き続き使用します。`,
		...
		unlearn: {
			sampledialogue: {
				...
				success_description: `対話ペアが正常に削除されました：ユーザー: "{input}" → ボット: "{output}"`,
			},
			attribute: {
				description: `Tomoriの記憶からパーソナリティ属性を削除します。`,
				modal_title: `属性を削除`,
				select_label: `削除する属性`,
				select_description: `Tomoriのパーソナリティから削除する属性を選択してください`,
			...
			servermemory: {
				...
				modal_title: `サーバー記憶を削除`,
				select_label: `削除する記憶`,
				select_description: `削除するサーバー記憶を選択してください`,
				...
				no_owned_memories: `削除できるサーバー記憶をお持ちではありません。`,
				memory_not_found: `選択された記憶が見つかりませんでした。`,
				select_title: `サーバー記憶を削除`,
				memory_label: `サーバー記憶`,
				success_title: `サーバー記憶が削除されました`,
				success_description: `サーバー記憶が正常に削除されました："{memory}"`,
			},
			personalmemory: {
				...
				memory_label: `個人記憶`,
				success_title: `個人記憶が削除されました`,
				success_description: `個人記憶が正常に削除されました："{memory}"`,
