...
export default {
  general: {
    ...
    pagination: {
      ...
      select_page_description: `{totalItems}個の項目を含む{totalPages}ページから、表示するページを選択してください：`,
    ...
    errors: {
      ...
      tomori_not_setup_description: `このサーバーではまだ私の設定が行われていないようです。\`Manage Server\`権限を持つメンバーが最初に\`/config setup\`を使用する必要があります。また、\`/help setup\`でヘルプを確認するか、\`/config language\`コマンドで言語設定を変更できます。`,
  ...
  genai: {
    ...
    message_interaction: {
      ...
      reply_context_footer: `{user}に返信中`,
    },
...
\`\`\``,
      ...
      image: {
        ...
        generate: {
          ...
          orientation_choice_portrait: `縦向き`,
          orientation_choice_landscape: `横向き`,
...
\`\`\``,
    ...
    bot: {
      ...
      respond: {
        ...
        main_persona_description: `メインペルソナ`,
        alter_persona_description: `オルタペルソナ`,
        embed_hide_notice: `ヒント: \`/config bot-permissions\` で「応答埋め込みを非表示」権限を有効にすると、この埋め込みを非表示にできます。`,
...
• ドキュメントの改善`,
    ...
    donate: {
      kofi: {
        ...
        description_text: `TomoriBotを使うのが好きですか？継続的な開発をサポートしてください！
...
**プロンプト:** {prompt}`,
  ...
  events: {
    addBot: {
      ...
      rejoin_description: `このサーバーに再追加されたようです。以前の設定と人格はそのままです！\`/config\`、\`/persona\`、\`/memory\`、\`/server\`コマンドで私を管理できます。データはいつでも\`/memory personal export\`、\`/memory server export\`、\`/personal config\`、\`/server config\`でエクスポート・リセットできます。
      ...
      setup_prompt_description: `追加してくれてありがとうございます！始めるには、**サーバー管理**権限を持つ方が\`/config setup\`コマンドを実行して、私の初期の人格を選択し、AI機能を設定する必要があります。データはいつでも\`/memory personal export\`、\`/memory server export\`、\`/personal config\`、\`/server config\`でエクスポート・リセットできます。
