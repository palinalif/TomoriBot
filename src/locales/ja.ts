...
2. PCでApplication Default Credentialsを設定：
   - Google Cloud CLIをインストール
   - \`gcloud auth application-default login\`を実行してブラウザからログイン
3. 次のコマンドを実行してプロジェクトIDを確認：
   - \`gcloud projects list\`
4. {configSetup}または{configApikeySet}で\`{project_id}::{location}\`の形式で設定
