...
export default {
	general: {
		...
		message_cooldown: `**{botName}** に再度話しかけるには **{seconds}** 秒待つ必要があります。`,
		message_cooldown_footer_per_user: `クールダウン: ユーザーごと`,
		message_cooldown_footer_per_channel: `クールダウン: チャンネルごと`,
		message_cooldown_footer_server_wide: `クールダウン: サーバー全体`,
		message_cooldown_footer_strict: `クールダウン: 厳密サーバー全体`,
		...
		config: {
			...
			cooldown: {
				type: {
					...
					desc_per_user: `各ユーザーが独自のクールダウンを持つ`,
					desc_per_channel: `各チャンネルが独自のクールダウンを持つ`,
				...
				length: {
					...
					success_disabled_description: `時間を **{length}** 秒に設定しましたが、クールダウンは現在 **無効** です。有効にするには \`/config cooldown type\` を使用してください。`,
					already_set_title: `既に設定済み`,
