...
- 自動トリガーチャンネルを制限する`,
			...
			status: {
				...
				server_page3_description: `自動チャット、RPチャンネル、ホワイトリスト、スケジュール済みランダムトリガー`,
		...
		config: {
			...
			randomtrigger: {
				add: {
					...
					channel_option: `自発的なメッセージを送信するチャンネル。`,
					timer_hours_option: `サイコロを振る頻度（時間単位、最低1時間）。`,
					random_offset_range_option: `タイマーがリセットされるたびに加算/減算するランダムオフセット範囲（時間、任意、最小0）。`,
					chance_option: `各ロールで発火する確率（1〜100%）。`,
					silence_threshold_option: `このX時間以内にチャンネルに活動があった場合はスキップ（任意）。`,
					...
					success_description: `{channel}を**{timer_hours}時間**ごとに確認し、**{chance}%**の確率で**{persona}**として発言します。{offset_suffix}{silence_suffix}`,
