    ...
    help: {
      ...
      speech: {
        ...
        qwen3tts: {
          ...
          steps_title: `設定手順`,
          steps_description: `**前提条件**: Python 3.10+、CUDA 12.x + ドライバー（任意、GPU 用）\n\n1. \`scripts/tts\` で Python \`.venv\` を作成して有効化します。\n2. \`requirements-qwen3tts.txt\` をインストールします。\n3. *(GPU のみ)* PyTorch を再インストールします: \`pip install torch torchaudio --index-url https://download.pytorch.org/whl/cu124\`\n4. \`qwen3tts_12hz_17b_base_server.py\` を起動します。\n5. {custom_endpoint_add} で capability \`speech\`、api_style \`tts-clone\`、script_markup \`plain\` として登録します。\n6. {model_speech} で選択し、{voice_add} と {voice_assign} を実行します。`,
        },
        irodoritts: {
          title: `IrodoriTTS 音声`,
          ...
          steps_title: `設定手順`,
          steps_description: `**前提条件**: Python 3.10+、CUDA 12.x + ドライバー（任意、GPU 用）\n\n1. \`scripts/tts\` で Python \`.venv\` を作成して有効化します。\n2. \`requirements-irodori.txt\` をインストールします。\n3. *(GPU のみ)* PyTorch を再インストールします: \`pip install torch torchaudio --index-url https://download.pytorch.org/whl/cu124\`\n4. \`irodori_tts_server.py\` を起動します。\n5. {custom_endpoint_add} で capability \`speech\`、api_style \`tts-clone\`、script_markup \`plain\` として登録します。\n6. {model_speech} で選択し、{voice_add} と {voice_assign} を実行します。`,
        },
        elevenlabs: {
          title: `ElevenLabs 音声`,
