  ...
  commands: {
    speech: {
      ...
      voice_remove: {
        ...
        select_sample_title: `削除する音声サンプルを選択`,
    ...
    config: {
      ...
      "api-key": {
        ...
        set: {
          ...
          novelai_success_with_model_description: `NovelAIのAPIキーが正常に検証、暗号化、保存されました。モデルは自動的に\`{model_name}\`に変更されました。⚠️ **絵文字とスタンプの使用は自動的に無効化されました**。NovelAIのコンテキストを安定させるためです。\`/config tool-use manage\`でいつでも再有効化できます。`,
...
現在 **{selected_count}** 件のチャンネルが有効です。`,
      ...
      "user-blacklist": {
        ...
        add: {
          ...
          personalization_disabled_description: `現在、サーバー全体でパーソナライズが無効になっています。まず \`/config tool-use manage\` で有効にしてください。`,
...
**警告:** 現在、このサーバーではパーソナライズが無効になっているため、この変更はここでの私の行動に影響しません。パーソナライズが有効になっている他のサーバーでは反映されます。`,
    ...
    generate: {
      image: {
        ...
        disabled_description: `このサーバーでは画像生成が無効になっています。\`/config tool-use manage\` で有効にできます（管理権限が必要）。`,
...
**プロンプト:** {prompt}`,
        ...
        disabled_description: `このサーバーでは動画生成が無効になっています。\`Manage Server\`権限を持つメンバーが \`/config tool-use manage\` で有効にできます。`,
        wrong_provider_title: `🔴 サポートされていないプロバイダー`,
