  ...
  genai: {
    ...
    stream: {
      ...
      prohibited_content_admin_notice_description:
        "確認: メッセージ(`/tool refresh`)、性格/記憶(`/data export`)、問題のあるメンバーをブラックリスト(`/server user-blacklist add`)、またはプロバイダを変更(`/config model`)",
      ...
      content_blocked_tip:
        "ヒント: `/config jailbreaks` でこのエラーの回避を試すか、メッセージ(`/tool refresh`)、性格/記憶(`/data export`)、問題のあるメンバーをブラックリスト(`/server user-blacklist add`)、またはプロバイダを変更(`/config model`)を確認してください。",
...
- ペルソナをエクスポートしてバックアップするか、他のサーバーと共有！`,
    ...
    conditioning: {
      ...
      manage: {
        description: `このサーバー内の全ペルソナに保存された条件付け履歴を管理します。`,
        marker_reward: `❤️`,
        marker_punish: `💀`,
        none_title: `管理する条件付けはありません`,
        none_description: `このサーバーには管理できる条件付け履歴がありません。`,
        ...
        select_page_description: `管理したい条件付け項目のページを選択してください。\n項目数: {total_entries}\nページ数: {total_pages}\n各項目にはペルソナ名とご褒美/おしおき種別が表示されます。`,
        checkbox_label: `条件付け項目`,
        checkbox_label_continued: `条件付け項目（続き）`,
        checkbox_description: `チェックを残すと保持されます。チェックを外すと、その保存済み条件付けグループが削除されます。`,
