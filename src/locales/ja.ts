...
export default {
  ...
  genai: {
    ...
    stream: {
      ...
      content_blocked_tip:
        "ヒント: `/nsfw jailbreaks` でこのエラーの回避を試すか、メッセージ(`/tool refresh`)、記憶/設定(`/memory personal export`、`/memory server export`、`/server config export`)、問題のあるメンバーをブラックリスト(`/server user-blacklist add`)、またはプロバイダを変更(`/config model`)を確認してください。",
