...
**警告:** 現在、このサーバーではパーソナライズが無効になっているため、この変更はここでの私の行動に影響しません。パーソナライズが有効になっている他のサーバーでは反映されます。`,
	...
	reminders: {
		...
		// 繰り返しリマインダーの案内（繰り返し実行後に送信）
		reminder_recurring_title: `🔁 繰り返しリマインダー`,
		reminder_recurring_description: `このリマインダーは{repetition_interval_hours}時間ごとに繰り返されます。`,
		reminder_recurring_footer: `リマインダーは\`/forget reminder\`で削除できます。`,
