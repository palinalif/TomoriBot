...
4. (任意) 必要に応じて\`/server avatar\`でアバターを変更`,
    ...
    help: {
      ...
      custom_models: {
        ...
        comfyui_page1_register_value: `サーバー共通なら {server_add_command}、個人用なら {personal_add_command} を使います。\`endpoint_url\` には ComfyUI サーバーの URL（例: \`http://127.0.0.1:8188\`）を入れ、\`api_style\` は \`ComfyUI\`、\`capability\` は \`Image\` か \`Video\` を選び、書き出した JSON を \`workflow_json\` に添付してください。その後、{image_command} または {video_command} でラベルを選択します。\`model_name\` は TomoriBot 側で表示するラベル名です。`,
      ...
      speech: {
        ...
        chatterbox: {
          ...
          description: `Chatterbox-Turbo は、参照音声クローンと角括弧タグを使いたい場合の推奨ローカル経路です。`,
          steps_title: `設定手順`,
          steps_description: `1. \`scripts/tts\` で Python \`.venv\` を作成して有効化します。\n2. \`requirements-chatterbox-turbo.txt\` をインストールし、\`chatterbox_turbo_server.py\` を起動します。\n3. {custom_endpoint_add} で capability \`speech\`、api_style \`tts-clone\`、script_markup \`bracket-tags\` として登録します。\n4. {model_speech} で選択し、{voice_add} と {voice_assign} を実行します。`,
        },
        qwen3tts: {
          title: `Qwen3-TTS 音声`,
          description: `Qwen3-TTS 12Hz Base は、Phase 4 ではプレーンな読み上げテキストを想定するローカル参照音声クローンです。`,
          steps_title: `設定手順`,
          steps_description: `1. \`scripts/tts\` で Python \`.venv\` を作成して有効化します。\n2. \`requirements-qwen3tts.txt\` をインストールし、\`qwen3tts_12hz_17b_base_server.py\` を起動します。\n3. {custom_endpoint_add} で capability \`speech\`、api_style \`tts-clone\`、script_markup \`plain\` として登録します。\n4. {model_speech} で選択し、{voice_add} と {voice_assign} を実行します。`,
        },
        irodoritts: {
          title: `IrodoriTTS 音声`,
          description: `IrodoriTTS は日本語向けのローカルクローン経路で、Phase 4 ではプレーンな読み上げテキストを想定します。`,
          steps_title: `設定手順`,
          steps_description: `1. \`scripts/tts\` で Python \`.venv\` を作成して有効化します。\n2. \`requirements-irodori.txt\` をインストールし、\`irodori_tts_server.py\` を起動します。\n3. {custom_endpoint_add} で capability \`speech\`、api_style \`tts-clone\`、script_markup \`plain\` として登録します。\n4. {model_speech} で選択し、{voice_add} と {voice_assign} を実行します。`,
        },
        elevenlabs: {
          title: `ElevenLabs 音声`,
          description: `ElevenLabs も同じ音声エンドポイント機構を使いますが、ショートカットコマンドで設定できます。`,
          steps_title: `設定手順`,
          steps_description: `{elevenlabs} に ElevenLabs APIキーを指定して実行します。音声生成と文字起こしエンドポイントを登録・選択し、必要なら1つのペルソナにすぐ声を割り当てます。追加ペルソナには後で {voice_assign} を使います。`,
        },
      },
      transcription: {
        ...
        whisperx: {
          ...
          description: `WhisperX は Phase 4 の推奨ローカルSTT経路です。`,
          steps_title: `設定手順`,
          steps_description: `1. \`scripts/stt\` で Python \`.venv\` を作成して有効化します。\n2. \`requirements-whisperx.txt\` をインストールし、\`whisperx_server.py\` を起動します。\n3. {custom_endpoint_add} で capability \`transcription\`、api_style \`openai-compatible-transcription\`、model \`large-v3\` などを指定して登録します。\n4. {model_transcription} で選択します。`,
        },
        whispercpp: {
          title: `whisper.cpp 文字起こし`,
          description: `whisper.cpp は OpenAI互換の文字起こしエンドポイントとして提供される場合に利用できます。`,
          steps_title: `設定メモ`,
          steps_description: `whisper.cpp HTTP サーバーを起動し、\`/v1/audio/transcriptions\` を提供していることを確認してから {custom_endpoint_add} で登録し、{model_transcription} で選択します。形が違う場合は小さなラッパーを挟んでください。`,
        },
        koboldcpp: {
    ...
    config: {
      ...
      custom_models: {
        ...
        add: {
          ...
          model_name_description: `正確なリモートモデルID。一部のエンドポイントでは完全一致のコード名が必要です。`,
          ...
          speech_script_markup_description: `このTTSエンドポイントが対応する音声マークアップ形式。`,
          speech_supports_instruct_description: `このTTSエンドポイントがスタイル指示を受け付けるか。`,
          transcription_model_description: `文字起こしエンドポイントへ送るモデルID。`,
          transcription_language_description: `任意の既定言語ヒント。例: en または ja。`,
        ...
        edit: {
          description: `既存のラベル付きカスタムエンドポイントの項目を置き換えます。`,
          endpoint_url_description: `選んだエンドポイントに使う新しいベースURLです。`,
          api_style_description: `選んだエンドポイントの新しいAPI形式です。`,
          model_name_description: `選んだエンドポイントの新しいリモートモデルIDです。`,
          display_name_description: `状態表示や確認画面で使う新しい表示名です。`,
          auth_token_description: `新しいBearerトークンです。省略すると現在のトークンを保持します。`,
          num_ctx_description: `テキストエンドポイント用の新しいコンテキストウィンドウ上書きです。`,
          has_tools_description: `そのテキストエンドポイントのツール呼び出し対応フラグを置き換えます。`,
          sees_images_description: `そのテキストエンドポイントの画像入力対応フラグを置き換えます。`,
          supports_structoutput_description: `そのテキストエンドポイントの構造化出力対応フラグを置き換えます。`,
          workflow_description: `画像/動画エンドポイント用のComfyUIワークフローJSONを置き換えます。`,
          none_title: `登録済みカスタムエンドポイントがありません`,
          none_description: `このサーバーには編集できるラベル付きカスタムエンドポイントがまだありません。`,
          select_modal_title: `カスタムエンドポイントを編集`,
          select_label: `置き換えるエンドポイント`,
          select_description: `どの登録済みカスタムエンドポイントに入力した値を反映するか選んでください。`,
          select_placeholder: `カスタムエンドポイントを選択...`,
          no_changes_title: `更新内容がありません`,
          no_changes_description: `編集する前に、少なくとも1つ置き換える項目を指定してください。`,
...
今回再表示した項目（{restored_count}）: {restored_list}`,
      ...
      speech: {
        ...
        elevenlabs: {
          ...
          assign_voice_description: `すぐに1つのペルソナへ声を割り当てるかどうか。`,
          ...
          voice_fetch_failed_title: `音声一覧を取得できませんでした`,
          voice_fetch_failed_description: `ElevenLabs は接続されましたが、音声一覧を読み込めませんでした。後で \`/config speech voice-assign\` から割り当てできます。`,
          select_persona_title: `音声を設定するペルソナを選択`,
          voice_modal_title: `ElevenLabs 音声を選択`,
          skip_voice_label: `音声割り当てをスキップ`,
          skip_voice_description: `今は接続だけ行い、音声は後で割り当てます。`,
          success_title: `ElevenLabs を接続しました`,
          success_skip_description: `ElevenLabs の音声生成と文字起こしエンドポイントを接続しました。ペルソナ音声は後で \`/config speech voice-assign\` から割り当てできます。`,
          success_assigned_description: `ElevenLabs を接続しました。**{persona}** は今後、ボイスメッセージで **{voice}** を使用します。`,
        },
        transcripts: {
        ...
        voice_add: {
          ...
          duplicate_error_title: `音声サンプルは既にあります`,
          duplicate_error_description: `このサーバーには既にローカル音声サンプルがあります。追加する前に削除してください。`,
          duration_error_title: `音声クリップが長すぎます`,
          duration_error_description: `音声サンプルは {limit_secs} 秒以下にしてください。`,
          success_title: `音声サンプルを追加しました`,
          success_description: `**{name}** をこのサーバーのローカル音声サンプルとして追加しました。\n\n長さ: {duration}\n参照テキスト: {ref_text_hint}`,
        ...
        voice_assign: {
          ...
          assign_clone_description: `このサーバーにアップロード済みの音声サンプルを使います。`,
...
セットアップ手順、Matrix側で使えるコマンド、現在の制限一覧は {help_matrix} を確認してください。`,
    ...
    personal: {
      ...
      custom_models: {
        ...
        edit: {
          description: `既存の個人用カスタムエンドポイントの項目を置き換えます。`,
          endpoint_url_description: `選んだ個人用エンドポイントに使う新しいベースURLです。`,
          api_style_description: `選んだ個人用エンドポイントの新しいAPI形式です。`,
          model_name_description: `選んだ個人用エンドポイントの新しいリモートモデルIDです。`,
          display_name_description: `状態表示や確認画面で使う新しい表示名です。`,
          auth_token_description: `新しいBearerトークンです。省略すると現在のトークンを保持します。`,
          num_ctx_description: `テキストエンドポイント用の新しいコンテキストウィンドウ上書きです。`,
          has_tools_description: `そのテキストエンドポイントのツール呼び出し対応フラグを置き換えます。`,
          ...
          select_label: `置き換えるエンドポイント`,
          select_description: `どの個人用カスタムエンドポイントに入力した値を反映するか選んでください。`,
...
**警告:** 現在、このサーバーではパーソナライズが無効になっているため、このニックネームはここでは使用しません。パーソナライズが有効になっている他のサーバーでは使用します。`,
    ...
    "scheduled-task": {
      ...
      edit: {
        ...
        select_option_description: `{reminder_time} ({timezone}) に {target_channel} で実行 | {reminder_type}{repeat_text}{manager_created_by_text}`,
