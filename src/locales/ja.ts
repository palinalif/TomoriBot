...
export default {
  ...
  genai: {
    ...
    image: {
      ...
      notice_inpaint_preset_line: `インペイントプリセット: {preset}`,
      notice_reference_line: `参照元: {message_url}`,
...
{model_list}`,
      ...
      "image-parameters": {
        description: `ComfyUI画像生成パラメーターを調整します。`,
        target_description: `調整する画像生成パスを選択してください。`,
        target_img2img: `画像から画像`,
        target_tight: `タイトなインペイント`,
        target_balanced: `バランス型インペイント`,
        target_loose: `ルーズなインペイント`,
        denoise_description: `Denoise 0〜1。既定: img2img .40、tight .30、balanced .80、loose .90。`,
        cfg_description: `インペイントCFG。既定: tight 8、balanced/loose 6。`,
        mask_threshold_description: `CLIPSeg閾値。既定: tight .45、balanced .28、loose .22。`,
        mask_grow_description: `マスク拡張px。既定: tight 8、balanced 32、loose 48。`,
        mask_feather_description: `マスクぼかし。既定: tight 8、balanced 20、loose 28。`,
        reset_description: `この対象をenv/デフォルト値に戻します。`,
        denoise_label: `denoise`,
        cfg_label: `cfg`,
        mask_threshold_label: `threshold`,
        mask_grow_label: `grow`,
        mask_feather_label: `feather`,
        reset_label: `reset`,
        no_changes_title: `変更するパラメーターがありません`,
        no_changes_description: `少なくとも1つのパラメーターを指定するか、resetで保存済み上書きをクリアしてください。`,
        unsupported_title: `非対応の画像エンドポイント`,
        unsupported_description: `これらのパラメーターを調整するには、有効な通常画像モデルがサーバーのComfyUIカスタムエンドポイントである必要があります。`,
        img2img_masks_title: `この設定にはインペイントが必要です`,
        img2img_masks_description: `ここでは画像から画像はdenoiseのみ使用します。CFGやマスク設定はインペイント対象で変更してください。`,
        success_title: `画像パラメーターを更新しました`,
        success_description: `**{target}** の設定を更新しました: {settings}`,
      },
      "image-preset": {
        description: `ComfyUIインペイントのカスタムプリセットを作成・更新します。`,
        name_description: `プリセット名。小文字、数字、ハイフン、アンダースコアを使用してください。`,
        preset_description_description: `モデルがこのプリセットを使う場面。例: 強い髪色変更、控えめな小物編集。`,
        denoise_description: `インペイントdenoise強度、0〜1。`,
        cfg_description: `インペイントCFGガイダンス強度。`,
        mask_threshold_description: `このプリセットのCLIPSeg閾値。`,
        mask_grow_description: `マスク拡張量（px）。`,
        mask_feather_description: `マスクぼかし/フェザー量。`,
        remove_description: `保存せず、このカスタムプリセットを削除します。`,
        invalid_name_title: `無効なプリセット名`,
        invalid_name_description: `1〜32文字の小文字、数字、ハイフン、アンダースコアを使用し、先頭は文字または数字にしてください。`,
        builtin_name_title: `組み込みプリセットです`,
        builtin_name_description: `組み込みプリセットは \`/model image-parameters\` で調整してください。カスタム名は tight、balanced、loose 以外にしてください。`,
        missing_values_title: `プリセット値が必要です`,
        missing_values_description: `作成・更新するには description、denoise、cfg、mask_threshold、mask_grow、mask_feather を指定してください。すべての値がある既存プリセットは、後から1項目ずつ更新できます。`,
        unsupported_title: `非対応の画像エンドポイント`,
        unsupported_description: `カスタムプリセットを保存するには、有効な通常画像モデルがサーバーのComfyUIカスタムエンドポイントである必要があります。`,
        success_title: `画像プリセットを保存しました`,
        success_description: `カスタムプリセット \`{name}\` を保存しました。\n説明: {description}\n設定: {settings}`,
        removed_title: `画像プリセットを削除しました`,
        removed_description: `カスタムプリセット \`{name}\` を削除しました。`,
      },
      video: {
