...
4. (任意) 必要に応じて\`/server avatar\`でアバターを変更`,
    ...
    help: {
      ...
      speech: {
        ...
        title: `音声生成の設定`,
        description_body: `音声エンドポイントを使うと、ローカル音声クローンまたはElevenLabsでDiscordボイスメッセージを送信できます。`,
        setup_field: `設定手順`,
        setup_value: `音声エンドポイントを登録し、ローカルクローンでは音声サンプルを追加してから、\`/config speech voice-assign\` でペルソナに割り当てます。`,
      },
      transcription: {
        description: `音声文字起こしの設定方法を確認します。`,
        title: `文字起こしの設定`,
        description_body: `文字起こしエンドポイントは、ユーザーの音声添付を会話コンテキスト用のテキストに変換します。`,
        setup_field: `設定手順`,
        setup_value: `OpenAI互換の文字起こしエンドポイントを登録し、サーバーの文字起こしモデルとして選択しておきます。`,
      },
      "custom-endpoint": {
        description: `カスタムエンドポイントの使い方を確認します。`,
      },
