...
3. Or select a different model that matches your privacy preferences`,
    ...
    anthropic: {
      ...
      temperature_top_p_conflict_message: `Anthropic rejected this request because both Temperature and Top-P were sent. Use \`/config params manage\` and uncheck either **Temperature** or **Top-P** for the current provider.`,
...
**Warning:** Personalization is currently disabled on this server, so I won't use this nickname here. I'll still use it on other servers where personalization is enabled.`,
      ...
      deliberatetriggermode: {
        description: `Toggle deliberate trigger mode (DTM) for yourself globally.`,
        enabled_title: `Personal DTM Enabled`,
        enabled_description: `You now require direct invocations to trigger any persona: \`@{trigger}\` prefix, replies, Discord mentions, or \`/bot respond\`. Plain trigger words will no longer work for you. If the server already has DTM enabled, this has no additional effect.`,
        disabled_title: `Personal DTM Disabled`,
        disabled_description: `You can trigger personas again with plain trigger words (unless the server has DTM enabled).`,
      },
    },
    "scheduled-task": {
