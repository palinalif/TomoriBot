...
4. (Optional) Use \`/server avatar\` to change the avatar if desired`,
    ...
    help: {
      ...
      custom_models: {
        ...
        comfyui_page1_description: `This guide assumes ComfyUI is already installed and running. Page 1 covers the minimum setup to reach a working \`/config custom-endpoint add\` or \`/personal custom-endpoint add\` registration.`,
        ...
        comfyui_page1_register_value: `Run {server_add_command} for a server-wide endpoint or {personal_add_command} for your own. Use your ComfyUI server URL for \`endpoint_url\` (for example \`http://127.0.0.1:8188\`), pick \`ComfyUI\` as \`api_style\`, choose \`Image\` or \`Video\` as the capability, attach the exported JSON as \`workflow_json\`, then select the label from {image_command} or {video_command}. The \`model_name\` field is just TomoriBot's display label for this endpoint.`,
      ...
      speech: {
        ...
        chatterbox: {
          ...
          description: `Chatterbox-Turbo is the recommended local path when you want bracket-style delivery tags with reference-audio cloning.`,
          steps_title: `Setup Steps`,
          steps_description: `1. Create and activate a Python \`.venv\` in \`scripts/tts\`.\n2. Install \`requirements-chatterbox-turbo.txt\` and start \`chatterbox_turbo_server.py\`.\n3. Register it with {custom_endpoint_add}: capability \`speech\`, api_style \`tts-clone\`, script_markup \`bracket-tags\`.\n4. Select it with {model_speech}, then run {voice_add} and {voice_assign}.`,
        },
        qwen3tts: {
          title: `Qwen3-TTS Speech`,
          description: `Qwen3-TTS 12Hz Base is a local reference-audio clone path that expects plain speech text in Phase 4.`,
          steps_title: `Setup Steps`,
          steps_description: `1. Create and activate a Python \`.venv\` in \`scripts/tts\`.\n2. Install \`requirements-qwen3tts.txt\` and start \`qwen3tts_12hz_17b_base_server.py\`.\n3. Register it with {custom_endpoint_add}: capability \`speech\`, api_style \`tts-clone\`, script_markup \`plain\`.\n4. Select it with {model_speech}, then run {voice_add} and {voice_assign}.`,
        },
        irodoritts: {
          title: `IrodoriTTS Speech`,
          description: `IrodoriTTS is the Japanese-focused local clone path and expects plain speech text in Phase 4.`,
          steps_title: `Setup Steps`,
          steps_description: `1. Create and activate a Python \`.venv\` in \`scripts/tts\`.\n2. Install \`requirements-irodori.txt\` and start \`irodori_tts_server.py\`.\n3. Register it with {custom_endpoint_add}: capability \`speech\`, api_style \`tts-clone\`, script_markup \`plain\`.\n4. Select it with {model_speech}, then run {voice_add} and {voice_assign}.`,
        },
        elevenlabs: {
          ...
          steps_title: `Setup Steps`,
          steps_description: `Run {elevenlabs} with your ElevenLabs API key. It registers speech and transcription endpoints, selects them, and can immediately assign a voice to one persona. Use {voice_assign} later for additional personas.`,
        },
      },
      transcription: {
        ...
        docs_title: `Full Docs`,
        docs_description: `See \`docs/integrations/transcription/\` and \`scripts/stt/README.md\` for local server setup notes.`,
        overview: {
        ...
        whisperx: {
          ...
          description: `WhisperX is the recommended local STT path for Phase 4.`,
          steps_title: `Setup Steps`,
          steps_description: `1. Create and activate a Python \`.venv\` in \`scripts/stt\`.\n2. Install \`requirements-whisperx.txt\` and start \`whisperx_server.py\`.\n3. Register it with {custom_endpoint_add}: capability \`transcription\`, api_style \`openai-compatible-transcription\`, and a model such as \`large-v3\`.\n4. Select it with {model_transcription}.`,
        },
        whispercpp: {
          title: `whisper.cpp Transcription`,
          description: `whisper.cpp can work when served through an OpenAI-compatible transcription endpoint.`,
          steps_title: `Setup Notes`,
          steps_description: `Start your whisper.cpp HTTP server, confirm it exposes \`/v1/audio/transcriptions\`, register it with {custom_endpoint_add}, then select it with {model_transcription}. If your server uses a different shape, place a small wrapper in front of it.`,
        },
        koboldcpp: {
    ...
    config: {
      ...
      custom_models: {
        ...
        add: {
          ...
          speech_script_markup_description: `Speech markup style supported by this TTS endpoint.`,
          speech_supports_instruct_description: `Whether this TTS endpoint accepts style instructions.`,
          transcription_model_description: `Model ID to send to the transcription endpoint.`,
          transcription_language_description: `Optional default language hint, such as en or ja.`,
          ...
          speech_next_steps_description: `Added **{display_name}** under label **{label}** for **{capability}**. Next: add a voice sample with \`/config speech voice-add\`, then assign it with \`/config speech voice-assign\`.`,
        },
        edit: {
          description: `Replace selected fields on an existing labeled custom endpoint.`,
          endpoint_url_description: `New base URL to use for the selected endpoint.`,
          api_style_description: `New API format for the selected endpoint.`,
          model_name_description: `New remote model ID for the selected endpoint.`,
          display_name_description: `New friendly name shown in status and confirmations.`,
          auth_token_description: `New bearer token. Leave omitted to keep the current token.`,
          num_ctx_description: `New context window override for text endpoints.`,
          has_tools_description: `Replace the text endpoint tool-calling flag.`,
          sees_images_description: `Replace the text endpoint vision-input flag.`,
          supports_structoutput_description: `Replace the text endpoint structured-output flag.`,
          workflow_description: `Replace the ComfyUI workflow JSON for image/video endpoints.`,
          ...
          select_label: `Endpoint To Replace`,
          select_description: `Choose which registered custom endpoint should receive the replacement values.`,
          select_placeholder: `Select a custom endpoint...`,
          no_changes_title: `Nothing To Update`,
          no_changes_description: `Pass at least one replacement field before choosing an endpoint to edit.`,
...
Restored now ({restored_count}): {restored_list}`,
      ...
      voice: {
        ...
        elevenlabs: {
          ...
          no_key_description: `An ElevenLabs key is required before choosing persona voices. Connect it first with \`/config speech elevenlabs\`.`,
      ...
      speech: {
        ...
        elevenlabs: {
          ...
          assign_voice_description: `Whether to assign a voice to one persona immediately.`,
          ...
          voice_fetch_failed_title: `Voice List Unavailable`,
          voice_fetch_failed_description: `ElevenLabs is connected, but I couldn't load the voice list. You can assign a voice later with \`/config speech voice-assign\`.`,
          select_persona_title: `Select Persona Voice Target`,
          voice_modal_title: `Select ElevenLabs Voice`,
          skip_voice_label: `Skip Voice Assignment`,
          skip_voice_description: `Connect ElevenLabs now and assign voices later.`,
          success_title: `ElevenLabs Connected`,
          success_skip_description: `ElevenLabs speech and transcription endpoints are connected. Assign a persona voice later with \`/config speech voice-assign\`.`,
          success_assigned_description: `ElevenLabs is connected. **{persona}** will now use **{voice}** for voice messages.`,
        },
        transcripts: {
        ...
        voice_add: {
          ...
          duplicate_error_title: `Voice Sample Already Exists`,
          duplicate_error_description: `This server already has a local voice sample. Remove it before adding another.`,
          ...
          success_description: `Added **{name}** as this server's local voice sample.\n\nDuration: {duration}\nReference text: {ref_text_hint}`,
          ref_text_provided: `provided`,
          ref_text_missing: `not provided`,
        },
        voice_remove: {
          description: `Remove this server's local TTS voice sample.`,
          no_sample_title: `No Voice Sample`,
          no_sample_description: `This server does not have a local voice sample to remove.`,
        ...
        voice_assign: {
          ...
          no_speech_endpoint_description: `Register a speech endpoint first with \`/config custom-endpoint add\`.`,
          no_sample_title: `No Voice Sample`,
          no_sample_description: `Add a local voice sample first with \`/config speech voice-add\`.`,
          select_persona_title: `Select Persona Voice Target`,
          clear_choice_label: `Disable Voice`,
          clear_choice_description: `Remove the current speech voice from this persona.`,
          assign_clone_title: `Assign Voice Sample`,
          assign_clone_description: `Use this server's uploaded local voice sample.`,
      ...
      model: {
        ...
        speech: {
          ...
          no_endpoints_description: `Register a speech endpoint first with \`/config custom-endpoint add\` or \`/config speech elevenlabs\`.`,
        ...
        transcription: {
          ...
          no_endpoints_description: `Register a transcription endpoint first with \`/config custom-endpoint add\` or \`/config speech elevenlabs\`.`,
...
⚠️ **Important:** Brave provides $5 in free monthly credits wherein usage beyond that is billed. To avoid unexpected charges, set a $5 usage limit in your [Brave usage limits dashboard](https://api-dashboard.search.brave.com/app/subscriptions/usage-limits).`,
      ...
      elevenlabs: {
        ...
        set: {
          ...
          success_voices_description: `Premade voices work on the free plan. Browse the full list at [ElevenLabs Premade Voices](https://elevenlabs-sdk.mintlify.app/voices/premade-voices), then use /config speech voice-assign to assign one to each persona.`,
          success_custom_voices_title: `Library & Custom Voices (Paid)`,
          success_custom_voices_description: `Library voices and custom/cloned voices both require a paid ElevenLabs plan. Once added to your account, they will appear automatically in /config speech voice-assign.`,
          success_transcript_mode_title: `Voice Transcript Mode`,
          success_transcript_mode_description: `Use /config speech transcripts to post voice message transcripts as visible chat messages via webhook.`,
        },
        remove: {
...
Use {help_matrix} for setup steps, Matrix-only command notes, and the current limitation list.`,
    ...
    personal: {
      ...
      custom_models: {
        ...
        edit: {
          description: `Replace selected fields on an existing personal custom endpoint.`,
          endpoint_url_description: `New base URL to use for the selected personal endpoint.`,
          api_style_description: `New API format for the selected personal endpoint.`,
          model_name_description: `New remote model ID for the selected personal endpoint.`,
          display_name_description: `New friendly name shown in status and confirmations.`,
          auth_token_description: `New bearer token. Leave omitted to keep the current token.`,
          num_ctx_description: `New context window override for text endpoints.`,
          has_tools_description: `Replace the text endpoint tool-calling flag.`,
          ...
          select_label: `Endpoint To Replace`,
          select_description: `Choose which personal custom endpoint should receive the replacement values.`,
          select_placeholder: `Select a personal custom endpoint...`,
          no_changes_title: `Nothing To Update`,
          no_changes_description: `Pass at least one replacement field before choosing a personal custom endpoint to edit.`,
...
**Warning:** Personalization is currently disabled on this server, so I won't use this nickname here. I'll still use it on other servers where personalization is enabled.`,
    ...
    "scheduled-task": {
      ...
      edit: {
        ...
        select_option_description: `At {reminder_time} ({timezone}) in {target_channel} | {reminder_type}{repeat_text}{manager_created_by_text}`,
