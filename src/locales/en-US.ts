...
4. (Optional) Use \`/server avatar\` to change the avatar if desired`,
    ...
    help: {
      ...
      custom_models: {
        ...
        comfyui_page1_description: `This guide assumes ComfyUI is already installed and running. Page 1 covers the minimum setup to reach a working \`/config custom-endpoint add\` or \`/personal custom-endpoint add\` registration. You may also use the ready-to-use /scripts/comfyui-workflows/ files inside the repo instead.`,
      ...
      speech: {
        ...
        chatterbox: {
          ...
          description: `Chatterbox-Turbo is a fast, lightweight English-only voice clone server. It supports bracket-style delivery tags such as \`[excited]\` or \`[whisper]\` to shape delivery — register it with \`script_markup = bracket-tags\` so TomoriBot passes those tags through intact.`,
          steps_title: `Setup Steps`,
          steps_description: `**Prerequisites**: Python 3.10+, CUDA 12.x + drivers (optional, for GPU)\n\n1. Create and activate a Python \`.venv\` in \`scripts/tts/chatterbox\`.\n2. Install numpy first (build dep): \`pip install numpy\`, then install \`requirements.txt\`.\n3. *(GPU only)* Reinstall PyTorch: \`pip install torch torchaudio --index-url https://download.pytorch.org/whl/cu124\`\n4. Start \`scripts/tts/chatterbox/server.py\`.\n5. Register with {custom_endpoint_add}: capability \`speech\`, api_style \`tts-clone\`, script_markup \`bracket-tags\`.\n6. Select with {model_speech}, then run {voice_add} and {voice_assign}.`,
        },
        qwen3tts: {
          title: `Qwen3-TTS Speech`,
          description: `Qwen3-TTS 12Hz Base is a multilingual voice clone server supporting 10 languages: Chinese, English, Japanese, Korean, German, French, Russian, Portuguese, Spanish, and Italian. It expects plain speech text only — no emotion markup. Register it with \`script_markup = plain\`.`,
          steps_title: `Setup Steps`,
          steps_description: `**Prerequisites**\n• Python 3.10+\n• SoX installed system-wide (required — \`scoop install sox\` on Windows, \`brew install sox\` on macOS; neither winget nor choco carry it)\n• CUDA 12.x + drivers (optional, for GPU)\n\n1. Create and activate a Python \`.venv\` in \`scripts/tts/qwen3tts\`.\n2. Install \`requirements.txt\`.\n3. *(GPU only)* Reinstall PyTorch: \`pip install torch torchaudio --index-url https://download.pytorch.org/whl/cu124\`\n4. *(Optional, GPU + Ampere/RTX 30xx+ only)* Install flash-attn for faster attention — requires CUDA torch (step 3), \`pip install wheel\`, then \`pip install flash-attn --no-build-isolation\`. Compiles from source and takes 20–40 min on Windows. Skip during initial setup.\n5. Start \`scripts/tts/qwen3tts/server.py\`.\n6. Register with {custom_endpoint_add}: capability \`speech\`, api_style \`tts-clone\`, script_markup \`plain\`.\n7. Select with {model_speech}, then run {voice_add} and {voice_assign}.`,
        },
        irodoritts: {
          title: `IrodoriTTS Speech`,
          description: `IrodoriTTS is a Japanese-specialized voice clone server. It reads emoji characters embedded in the speech text as emotion cues (e.g. 😊 for happy, 😢 for sad). Register it with \`script_markup = plain\` — TomoriBot strips bracket tags before sending, leaving only the emoji markers the model expects.`,
          steps_title: `Setup Steps`,
          steps_description: `**Prerequisites**: Python 3.10+, CUDA 12.x + drivers (optional, for GPU)\n\n1. Create and activate a Python \`.venv\` in \`scripts/tts/irodoritts\`.\n2. Install \`requirements.txt\`.\n3. Install irodori-tts via the patch script (upstream packaging bugs require this):\nWindows: \`scripts/tts/irodoritts/install-irodori.ps1\`\nLinux/macOS: \`bash scripts/tts/irodoritts/install-irodori.sh\`\n4. *(GPU only)* Reinstall PyTorch: \`pip install torch torchaudio --index-url https://download.pytorch.org/whl/cu124\`\n5. Start \`scripts/tts/irodoritts/server.py\`.\n6. Register with {custom_endpoint_add}: capability \`speech\`, api_style \`tts-clone\`, script_markup \`plain\`.\n7. Select with {model_speech}, then run {voice_add} and {voice_assign}.`,
        },
        elevenlabs: {
      ...
      transcription: {
        ...
        whisperx: {
          ...
          steps_title: `Setup Steps`,
          steps_description: `**Prerequisites**\n• Python 3.10+\n• FFmpeg installed system-wide (required for audio decoding)\n• CUDA 12.x + drivers (optional, for GPU acceleration)\n\n1. Create and activate a Python \`.venv\` in \`scripts/stt\`.\n2. Install \`requirements-whisperx.txt\`.\n3. *(GPU only)* Reinstall PyTorch with CUDA support:\n\`pip install torch torchaudio --index-url https://download.pytorch.org/whl/cu124\`\n4. Start \`whisperx_server.py\`.\n5. Register with {custom_endpoint_add}: capability \`transcription\`, api_style \`openai-compatible-transcription\`, and the model name matching your chosen size.\n6. Select with {model_transcription}.`,
    ...
    config: {
      ...
      custom_models: {
        ...
        capability_modal: {
          ...
          supports_instruct_description: `Check if this endpoint accepts instruction text alongside the speech script`,
...
Restored now ({restored_count}): {restored_list}`,
      ...
      model: {
        ...
        speech: {
          ...
          no_endpoints_description: `Register a speech endpoint first with \`/config custom-endpoint add\` or \`/speech elevenlabs\`.`,
        ...
        transcription: {
          ...
          no_endpoints_description: `Register a transcription endpoint first with \`/config custom-endpoint add\` or \`/speech elevenlabs\`.`,
