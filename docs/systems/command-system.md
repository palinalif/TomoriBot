...
- `server`: trigger(add/delete), whitelist(channel/role/remove), cooldown(triggers), autotrigger(*), matrix(link/unlink), quota(imagegen/textgen/reset), rpchannel(add/remove)
