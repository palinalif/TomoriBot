...
- Saved character references are still persisted by `/novelai charreference`, but the current active `generate_image_nai` character prompting flow does not inject profile-driven refs or profile-driven appearance tags.
