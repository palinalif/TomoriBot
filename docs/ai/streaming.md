...
- Adapter-level speaker fallback only runs when `/config stop-strings manage` enables the `\n{Name}:` speaker-pattern checkbox for that provider. When enabled, it stops on registered speaker labels already present in context, plus reserved `Assistant:` labels, while ignoring speaker-like lines inside fenced or inline backtick code.
- Shared stop-string rules live in `src/providers/utils/stopStrings.ts`. That registry handles universal stop strings, provider/model-specific stop strings, user-configured provider stop strings, and the opt-in persona speaker stop.

Stream-level safeguard:
- When the provider's speaker-pattern checkbox is enabled, `StreamOrchestrator` truncates any flushed segment at the first line that starts with a registered non-active speaker label (`Name:`) or reserved `Assistant:` label, then stops the stream. The same guard is skipped by default.
