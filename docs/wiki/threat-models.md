...
| **Information Disclosure** | `cross_channel_message` peeks into a channel and leaks recent messages back to another channel. | High | Runtime requires bot `ViewChannel`, and dispatch requires `SendMessages` or `SendMessagesInThreads`. The server cross-channel blocklist blocks configured channels and forum/media parents. | `src/tools/functionCalls/crossChannelMessageTool.ts`, `src/commands/server/crosschannel-blocklist.ts` | The tool follows the bot's channel visibility, not each human participant's visibility. Admins must block sensitive channels or restrict bot access. |
...
| Threat (STRIDE) | Scenario | Risk | Current Mitigation | Implementation Area | Residual Risk / Assumption |
| :--- | :--- | :--- | :--- | :--- | :--- |
| **Denial of Service** | Oversized or slow Discord attachment downloads exhaust memory or worker time. | High | `safeDownload()` pre-checks known size, enforces content-length and final size checks, and uses timeouts. | `src/utils/security/safeDownload.ts`, `src/utils/documents/textExtractor.ts`, `src/utils/audio/audioAttachmentTranscription.ts`, `src/utils/teach/batchUploadUtils.ts` | Some direct Discord CDN or provider-returned media fetches still use `fetch()` with local checks rather than the shared wrapper. |
...
| **Information Disclosure / SSRF** | Avatar or character-reference storage loads arbitrary remote URLs. | Medium | User upload paths generally go through Discord attachments and safe download; local stored paths are constrained under `data/avatars`, `data/charreferences`, and `data/voice-samples`. | `src/utils/storage/avatarStorage.ts`, `src/utils/storage/charrefStorage.ts`, `src/utils/storage/voiceSampleStorage.ts` | Some stored remote URLs are fetched directly when reloading generated assets, so keep stored asset URLs limited to Tomori-controlled S3/CloudFront or Discord CDN. |
...
| Cross-channel tool | `cross_channel_message` reads and dispatches based on bot channel visibility, not the invoking user's channel visibility. | Consider requiring invoker `ViewChannel` for peek and dispatch targets, or add an explicit allowlist mode for servers that want bot-wide visibility behavior. |
| User/admin-configured remote fetches | Some stored asset/provider/media paths still use direct `fetch()` or local ad hoc checks. | Audit direct `fetch()` calls reachable from user/admin-configured URLs and migrate them to `fetchUserRemoteUrl()` or `safeDownload()` as appropriate. |
