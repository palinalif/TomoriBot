...
| **Tampering** | **RAG / Shared Memory Data Poisoning:** A malicious user deliberately feeds false or malicious information into the chat. The bot saves this to its vector DB, later regurgitating it to other users in the same guild, poisoning the shared context. | Medium / High | Memory context scoping, decay over time, and user-specific isolation where appropriate. | `src/db/`, `src/ai/conditioning-memory.ts` | **Accepted Risk:** Guild-level memories inherently carry the risk of cross-contamination. We rely on Discord server moderators to manage bad actors and provide commands to clear poisoned memory (e.g., `/wipe_memory`). |
...
## 7. Supply Chain & Deployment Vectors

This vector covers risks introduced during the build, dependency resolution, and deployment processes.

| Threat (STRIDE) | Specific Vulnerability / Scenario | Risk | Mitigation Strategy | Implementation Area | Accepted Residual Risk & Assumptions |
| :--- | :--- | :--- | :--- | :--- | :--- |
| **Tampering / Elevation of Privilege** | **Malicious Dependency Update:** A compromised npm package or Python wheel introduces malware into the build process or runtime environment. | High | Strict lockfiles (`bun.lock`), explicit version pinning (no `@latest` in Docker), and cryptographic checksum verification for downloaded assets. | `Dockerfile`, `.github/workflows/deploy-tomoribot.yml`, `package.json` | **Accepted Risk:** We rely on the security of the upstream registries (npm, PyPI) and the integrity of the build tools (Bun, pip). We use CI/CD scanning (Trivy, Bun Audit) to catch known CVEs, but zero-day supply chain attacks remain a residual risk. |
| **Information Disclosure** | **Exposed CI/CD Secrets:** Deployment secrets (AWS credentials, DockerHub tokens) leak through workflow logs or compromised actions. | High | OIDC authentication for AWS (`role-to-assume`), avoiding static long-lived credentials, and automated secret scanning. | `.github/workflows/deploy-tomoribot.yml` | **Assumption:** GitHub Actions infrastructure and the OIDC provider remain secure and uncompromised. |
